...
class ReportfacturaView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'search_report':
                ...
                search = Sale.objects.all().filter(user_creation_id=get_current_user())
                #datejoined es como el between en sql 
            	#si hay un valor en inicio y fin se realiza 
                if len(start_date) and len(end_date):
                    search = search.filter(date_joined__range=[start_date, end_date])
                for s in search:
                	#data contiene el array 
                    data.append([
                        s.id,
                        s.cli.names,
